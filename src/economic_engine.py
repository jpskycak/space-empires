...
class EconomicEngine:
    ...
    def complete_all_taxes(self, turn):
        for player in self.game.players:
            ...
            if not isinstance(purchase, int): #if its not an upgrade
                ship = self.create_ship(player, purchase, player.strategy.decide_ship_placement(self.game.game_state), player.grid_size, len(player.ships))
                while player.creds >= ship.cost: 
                    purchase = player.strategy.decide_ship_purchases(self.game.game_state)
                    ship = self.create_ship(player, purchase, player.strategy.decide_ship_placement(self.game.game_state), self.game.grid_size, len(player.ships))
    ...
    def create_ship(self, player, ship, position, grid_size, ID):
        if isinstance(ship, Scout): return Scout(player, position, grid_size, ID, True)
        elif isinstance(ship, Destroyer): return Destroyer(player, position, grid_size, ID, True)
        elif isinstance(ship, Cruiser): return Cruiser(player, position, grid_size, ID, True)
        elif isinstance(ship, BattleCruiser): return BattleCruiser(player, position, grid_size, ID, True)
        elif isinstance(ship, Battleship): return Battleship(player, position, grid_size, ID, True)
        elif isinstance(ship, Dreadnaught): return Dreadnaught(player, position, grid_size, ID, True)
        elif isinstance(ship, Carrier): return Carrier(player, position, grid_size, ID, True)
        elif isinstance(ship, Colony_Ship): return Colony_Ship(player, position, grid_size, ID, True)
        else: return Scout(player, position, grid_size, ID, True)
    ...
    def maintenance(self, player, turn):
        ...
        player.ships = [ship for ship in player.ships if ship.__dict__ not in removals]
        return total_cost