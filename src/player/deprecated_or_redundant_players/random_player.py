...
class RandomPlayer(Player):
    def __init__(self, position, board_size, player_index, player_color):
        ...
        self.ships = [
            Scout(1, position, self.board_size, True),
            Scout(2, position, self.board_size, True),
            Scout(3, position, self.board_size, True),
            Colony_Ship(4, position, self.board_size, True),
            Colony_Ship(5, position, self.board_size, True),
            Colony_Ship(6, position, self.board_size, True)
        ]
    ...
    def upgrade(self, stat_to_upgrade):
        if stat_to_upgrade == 1 and self.attack_tech < 3:  # offense
            ...
            print('Player', self.player_index, 'upgraded their movement speed from',
