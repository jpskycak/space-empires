...
class AttackBerserkerStrategyLevel2(BasicStrategy):
    def __init__(self, player_index):  # wutever else we need):
        self.player_index = player_index
        self.__name__ = 'AttackBerserkerStrategyLevel2'

    def decide_purchases(self, hidden_game_state):
        purchases = {'units': [], 'technology': []}
        total_cost = 0
        if hidden_game_state['turn'] == 1 and hidden_game_state['players'][self.player_index]['technology']['attack'] == 0 and 'attack' not in purchases['technology']:
            if hidden_game_state['players'][self.player_index]['cp'] > total_cost + self.upgrade_costs('attack', hidden_game_state):
                purchases['technology'].append('attack')
                total_cost += self.upgrade_costs('attack', hidden_game_state)
